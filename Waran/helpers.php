...
function seo($model, $model_id = null): App\Models\Seo
{
  if(is_object($model)) {
      $model_name = short_class_name($model);
      $model_id = $model->id;
  }
  else {
      $model_name = $model;
  ...
  $seo = \App\Models\Seo::where('model_name', $model_name)->where('model_id', $model_id)->first();
  seo_generate($seo->data);

  return $seo;
}

function seo_generate($seo_data)
{
    if(!$seo_data) return false;

    foreach($seo_data as $tag => $data)
    {
      $property = ($tag != 'keyword') ? 'set'.ucwords($tag) : 'add'.ucwords($tag);
      SEOMeta::{$property}($data);
    }
}
